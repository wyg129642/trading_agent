...
- **付费墙/内容缺失处理（极其重要）**：如果正文内容明显是付费墙页面（如登录提示、cookie声明、订阅广告、"Subscribe to read"等），或正文极短（少于200字符）且与标题无关，你必须**仅基于标题进行评估**，不要因为正文无实质内容就给低分。来自 The Information、TechCrunch、VentureBeat 等知名科技媒体的文章，即使正文被付费墙阻挡，标题本身就包含了足够的信息来判断市场相关性。例如"Nvidia's New Server Rack Will Run AI Chips Made by Rivals"即使没有正文，标题明确涉及NVIDIA和AI芯片竞争格局变化，应给予0.8+的评分"""
...
2. **逐标的多周期情绪判断 (per_stock_sentiment)**:
   ...
   三个时间周期（精确定义，用于量化回测）：
   - **short_term**: 下一个交易日收盘价相对于今日收盘价的涨跌（精确对应T+1回测）
   - **medium_term**: 第5个交易日收盘价相对于今日收盘价的涨跌（精确对应T+5回测）
   - **long_term**: 20个交易日后收盘价相对于今日收盘价的涨跌方向（对应T+20回测，反映长期趋势）

   每个周期只需要提供两个字段：
   - **sentiment**: 涨跌方向 bullish|bearish|neutral（只需这三个值，简洁明确）
   - **confidence**: 置信度 0.0到1.0。表示你对这个涨跌判断的把握程度。回测时将用此值作为交易门槛（如只执行confidence>0.6的信号）。必须严格校准：
     0.8-1.0: 高置信度 — 有多个独立信源交叉验证，有历史先例数据支撑，有明确的量化证据。confidence=0.9意味着你在10次类似判断中有9次是正确的。
     0.6-0.7: 中高置信度 — 有较充分证据但存在部分不确定性
     0.4-0.5: 中等置信度 — 证据有限，依赖推理而非硬数据
     0.2-0.3: 低置信度 — 信息严重不足，主要基于类比推测
     0.0-0.1: 极低置信度/纯猜测

     【校准锚点 — 严格遵守】
     - 如果只有标题信息没有正文详情：confidence ≤ 0.4
     - 如果缺少历史先例数据：confidence ≤ 0.5
     - 如果多个证据相互矛盾：confidence ≤ 0.3
     - 如果新闻来源不权威（自媒体、论坛）：confidence 下调0.1-0.2
     - 如果搜索结果丰富且多源一致：confidence 可上调0.1-0.2
...
【重要：基于证据的判断原则】
你的所有评估必须基于深度研究阶段收集到的实际证据。如果深度研究发现不充分（搜索结果少、无历史先例、缺少股价数据），你必须：
1. 将所有时间周期的confidence设为≤0.4
2. 将sentiment判断为neutral
3. 在summary中明确说明证据不足
不要在证据不充分时给出bullish/bearish信号。
...
示例1（高意外度 + 高置信度）：
某AI芯片公司突然宣布其下一代芯片性能超预期3倍，此前市场完全不知情。
→ surprise_factor: 0.85, short_term: {sentiment: "bullish", confidence: 0.8}
原因：完全超出市场预期，有官方公告作为硬证据，历史上类似突破性产品发布后股价平均上涨8-15%。

示例2（低意外度 + 高置信度）：
某公司发布Q3季报，营收同比增长15%，与分析师预期基本一致。
→ surprise_factor: 0.15, short_term: {sentiment: "neutral", confidence: 0.7}
原因：完全在预期内，市场已充分定价。confidence较高是因为"无意外=无影响"这个判断本身很可靠。

示例3（复杂案例：短期看空但长期看多）：
某医药公司核心产品临床试验失败，但公司宣布获得另一重磅产品的FDA突破性疗法认定。
→ short_term: {sentiment: "bearish", confidence: 0.7}
   medium_term: {sentiment: "neutral", confidence: 0.4}
   long_term: {sentiment: "bullish", confidence: 0.3}
原因：短期失败消息冲击确定性高(confidence=0.7)，但长期看多因新产品前景好但不确定性大(confidence=0.3)。"""
...
{{
  ...
  "per_stock_sentiment": [
    {{
      "ticker": "股票代码",
      "name": "股票名称",
      "short_term": {{"sentiment": "bullish|bearish|neutral", "confidence": 0.0到1.0}},
      "medium_term": {{"sentiment": "bullish|bearish|neutral", "confidence": 0.0到1.0}},
      "long_term": {{"sentiment": "bullish|bearish|neutral", "confidence": 0.0到1.0}},
      "reason": "不同时间维度的影响分析"
    }}
  ],
...
- 每个周期只需sentiment(bullish/bearish/neutral)和confidence两个字段，不需要sentiment_score。
...
2. **Per-Stock Multi-Horizon Sentiment (per_stock_sentiment)**:
   ...
   Three horizons (precise definitions for quantitative backtesting):
   - **short_term**: Next trading day's close relative to today's close (precisely maps to T+1 backtest)
   - **medium_term**: The 5th trading day's close relative to today's close (precisely maps to T+5 backtest)
   - **long_term**: The 20th trading day's close relative to today's close (maps to T+20 backtest, reflects longer-term trend)

   For each horizon, provide only two fields:
   - **sentiment**: direction (bullish|bearish|neutral — only these three values, keep it simple)
   - **confidence**: [0.0, 1.0]. How confident you are in this direction call. This will be used as a trading threshold in backtesting (e.g., only execute signals with confidence > 0.6). Must be rigorously calibrated:
     0.8-1.0: High confidence — multiple independent sources cross-validated, historical precedent data available, clear quantitative evidence. confidence=0.9 means you would be correct 9 out of 10 times on similar calls.
     0.6-0.7: Medium-high confidence — substantial evidence but some uncertainty remains
     0.4-0.5: Moderate confidence — limited evidence, relying on reasoning rather than hard data
     0.2-0.3: Low confidence — severely insufficient information, mostly based on analogical speculation
     0.0-0.1: Very low confidence / pure guesswork

     【Calibration Anchors — Strictly Follow】
     - If only headline available with no article details: confidence <= 0.4
     - If no historical precedent data found: confidence <= 0.5
     - If multiple pieces of evidence contradict each other: confidence <= 0.3
     - If news source is not authoritative (blogs, forums): reduce confidence by 0.1-0.2
     - If search results are rich and multi-source consistent: may increase confidence by 0.1-0.2
...
【IMPORTANT: Evidence-Based Judgment Principle】
All assessments must be grounded in actual evidence from the deep research phase. If research findings are insufficient (few search results, no historical precedents, missing price data), you MUST:
1. Set confidence ≤ 0.4 for all time horizons
2. Set sentiment to neutral
3. Explicitly state insufficient evidence in summary
Do NOT issue bullish/bearish signals without adequate supporting evidence.
...
【Reference Examples — Calibration Aid】

Example 1 (High surprise + High confidence):
An AI chip company suddenly announces its next-gen chip exceeds performance expectations by 3x, with no prior market awareness.
→ surprise_factor: 0.85, short_term: {sentiment: "bullish", confidence: 0.8}
Reasoning: Completely exceeded market expectations, official announcement as hard evidence, historically similar breakthrough product launches saw 8-15% average stock price gains.

Example 2 (Low surprise + High confidence):
A company reports Q3 earnings with 15% YoY revenue growth, in line with analyst estimates.
→ surprise_factor: 0.15, short_term: {sentiment: "neutral", confidence: 0.7}
Reasoning: Fully within expectations, market already priced in. Confidence is relatively high because "no surprise = no impact" is itself a reliable judgment.

Example 3 (Complex case: short-term bearish but long-term bullish):
A pharma company's core product fails clinical trials, but the company announces FDA Breakthrough Therapy designation for another blockbuster product.
→ short_term: {sentiment: "bearish", confidence: 0.7}
   medium_term: {sentiment: "neutral", confidence: 0.4}
   long_term: {sentiment: "bullish", confidence: 0.3}
Reasoning: Short-term failure news impact is highly certain (confidence=0.7), but long-term bullish on new product prospects with high uncertainty (confidence=0.3)."""
...
{{
  ...
  "per_stock_sentiment": [
    {{
      "ticker": "stock ticker",
      "name": "stock name",
      "short_term": {{"sentiment": "bullish|bearish|neutral", "confidence": 0.0 to 1.0}},
      "medium_term": {{"sentiment": "bullish|bearish|neutral", "confidence": 0.0 to 1.0}},
      "long_term": {{"sentiment": "bullish|bearish|neutral", "confidence": 0.0 to 1.0}},
...
Notes:
- per_stock_sentiment MUST provide three time horizons (short_term/medium_term/long_term) for each stock in 【Related Stocks】.
- Each horizon only needs sentiment (bullish/bearish/neutral) and confidence — no sentiment_score needed.
