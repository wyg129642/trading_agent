        ...
        for col, default in [
            ...
            try:
                ...
                if "duplicate column" not in str(e).lower():
                    logger.debug("ALTER TABLE analysis_results ADD %s: %s", col, e)
    ...
    async def save_analysis_result(self, result: AnalysisResult) -> None:
        await self._db.execute(
            ...
            (
                ...
                json.dumps(getattr(result, "concept_tags", []), ensure_ascii=False),
                json.dumps(getattr(result, "industry_tags", []), ensure_ascii=False),
                json.dumps(getattr(result, "ticker_sentiments", {}), ensure_ascii=False),
                json.dumps(getattr(result, "sector_sentiments", {}), ensure_ascii=False),
            ),
        )
        await self._db.commit()
