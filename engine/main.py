...
from engine.monitors.api_monitor import API_MONITOR_MAP
from engine.monitors.base import BaseMonitor
from engine.monitors.rss_monitor import RSSMonitor
from engine.monitors.web_scraper import WebScraperMonitor
...
class TradingAgent:
    ...
    def __init__(self):
        ...
        self.proactive_scanner = None  # ProactiveScanner (lazy import)
    ...
    async def initialize(self) -> None:
        ...
        # HTTP sessions — separate for domestic (direct) and overseas (proxy)
        max_conns = self.settings.get("system", {}).get("max_concurrent_requests", 20)
        proxy_url = os.getenv("HTTP_PROXY") or os.getenv("http_proxy") or "http://127.0.0.1:7890"

        connector_direct = aiohttp.TCPConnector(limit=max_conns, ttl_dns_cache=300)
        self.session_direct = aiohttp.ClientSession(connector=connector_direct, trust_env=False)

        connector_proxy = aiohttp.TCPConnector(limit=max_conns, ttl_dns_cache=300)
        self.session_proxy = aiohttp.ClientSession(connector=connector_proxy, trust_env=False)
        self._proxy_url = proxy_url

        # Default session (backward compat) — used by content_fetcher etc.
        self.session = self.session_direct

        logger.info("Proxy configured: overseas sources → %s, domestic → direct", proxy_url)

        # Headless browser (Playwright) — with proxy for overseas sites
        ...
        # Refresh THS concept tags from remote DB (non-blocking, falls back to snapshot)
        try:
            from config.tags import refresh_concepts
            await refresh_concepts()
        except Exception as e:
            logger.warning("Failed to refresh concept tags: %s — using snapshot", e)
        ...
        # Proactive portfolio scanner (optional)
        proactive_cfg = self.settings.get("proactive_monitoring", {})
        if proactive_cfg.get("enabled", False):
            try:
                ...
                await self.proactive_scanner.initialize()
                logger.info(
                    "Proactive scanner initialized: %d holdings",
                    len(self.proactive_scanner.holdings),
                )
            except Exception as e:
                logger.warning("Proactive scanner init failed (continuing without): %s", e)
                self.proactive_scanner = None
    ...
    def _update_health(self, status: str, message: str = "", **extra) -> None:
        ...
        try:
            self._health_file.write_text(json.dumps(health, ensure_ascii=False), encoding="utf-8")
        except Exception as e:
            logger.debug("Failed to write health file: %s", e)

    def _select_session(self, market: str) -> aiohttp.ClientSession:
        """Pick the right HTTP session: proxy for overseas, direct for China."""
        if market in ("china",):
            return self.session_direct
    ...
    def _create_monitors(self) -> None:
        ...
        for src_cfg in source_list:
            ...
            session = self._select_session(market)
    ...
    def _is_similar_to_recent(self, item) -> bool:
        ...
        for ts, src, prev_title, prev_cs in self._recent_titles:
            ...
            if union and len(intersection) / len(union) > 0.5:
                logger.info("[Pipeline] Similar (jaccard), skipping: '%s' ≈ '%s'", item.title[:40], prev_title[:40])
                return True
            if (is_data_release and topic_prefix and
                    src == item.source_name and prev_title.startswith(topic_prefix)):
                if bool(_re.search(r"[前值预期]\s*[\d.]", prev_title)):
                    logger.info("[Pipeline] Same-topic data batch, skipping: '%s' ≈ '%s'", item.title[:40], prev_title[:40])
                    return True
    ...
    async def run(self) -> None:
        ...
        if self.proactive_scanner:
            self._tasks.append(asyncio.create_task(
                self.proactive_scanner.run(), name="proactive_scanner"
            ))
    ...
    async def shutdown(self) -> None:
        ...
        if self.proactive_scanner:
            await self.proactive_scanner.shutdown()
        ...
        for sess in (getattr(self, "session_direct", None),
                     getattr(self, "session_proxy", None)):
            if sess:
                await sess.close()
        if self.db:
            await self.db.close()
