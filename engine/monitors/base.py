...
from engine.models import NewsItem, SourceHealth
...
class BaseMonitor(ABC):
    ...
    def _needs_proxy(self) -> bool:
        """Return True if this source is overseas and should use proxy."""
        return self.market not in ("china",)
    ...
    async def _get(self, url: str, **kwargs) -> str:
        ...
        # Overseas sources use proxy via environment (session is pre-configured)
        proxy = None
        if self._needs_proxy():
            import os
            proxy = os.getenv("HTTP_PROXY") or os.getenv("http_proxy") or "http://127.0.0.1:7890"
        async with self.session.get(url, headers=headers, timeout=timeout, ssl=False, proxy=proxy, **kwargs) as resp:
    ...
    async def _get_html(self, url: str) -> str:
        ...
        if self.config.get("requires_browser") and self.browser_manager:
            ...
            logger.debug("[%s] Using Playwright for %s (timeout=%dms, wait=%s)", self.name, url, timeout_ms, wait_until)
            return await self.browser_manager.fetch_page(
                url,
                timeout_ms=timeout_ms,
                use_proxy=self._needs_proxy(),
