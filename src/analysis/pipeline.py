...
from src.analysis.prompts import get_prompts
from src.analysis.signal_scorer import score_signal
...
class AnalysisPipeline:
    ...
    async def process(
        ...
        # === Auto-translate non-Chinese titles ===
        await self._translate_title_if_needed(item)
        ...
        # Neutral items are marked as not relevant so they won't appear in signal evaluation
        is_relevant = evaluation.may_affect_market and not evaluation.is_neutral
        filter_reason = evaluation.reason
        if evaluation.is_neutral:
            filter_reason = f"[中性] {evaluation.reason}"
        ...
        # Gate check: relevance and market impact
        ...
        # Gate check: neutrality — neutral news skips deep research
        if evaluation.is_neutral:
            if test_mode:
                logger.info(
                    "[Pipeline][TEST] Bypassing neutral filter: %s",
                    item.title[:50],
                )
            else:
                logger.info(
                    "[Pipeline] Neutral news, skipping deep research: %s (score=%.2f)",
                    item.title[:50], evaluation.relevance_score,
                )
                return result

        # === Stale article check (skipped in test mode) ===
        if not test_mode and not debug_trace and self.max_article_age_hours > 0:
            from datetime import timezone
            now = datetime.now(timezone.utc)

            # Check 1: Use published_at if available
            if item.published_at:
                ...
                if age_hours > self.max_article_age_hours:
                    logger.info(
                        "[Pipeline] Stale article (%.0fh old, published_at): %s",
                        age_hours, item.title[:50],
                    )
                    return result

            # Check 2: Use LLM's is_stale / estimated_publish_date from Phase 1
            if evaluation.is_stale:
                llm_pub_date = evaluation.estimated_publish_date
                if llm_pub_date and str(llm_pub_date) != "null":
                    try:
                        est_pub = datetime.strptime(str(llm_pub_date)[:10], "%Y-%m-%d")
                        est_pub = est_pub.replace(tzinfo=timezone.utc)
                        age_hours = (now - est_pub).total_seconds() / 3600
                        if age_hours > self.max_article_age_hours:
                            logger.info(
                                "[Pipeline] Stale article (LLM estimated %.0fh old, date=%s): %s",
                                age_hours, llm_pub_date, item.title[:50],
                            )
                            return result
                    except (ValueError, TypeError):
                        pass
                else:
                    # LLM says stale but no date — trust LLM judgment
                    logger.info(
                        "[Pipeline] Stale article (LLM flagged is_stale=true): %s",
                        item.title[:50],
                    )
                    return result
    ...
    async def _translate_title_if_needed(self, item: NewsItem) -> None:
        """Translate non-Chinese news titles to Chinese via LLM.

        Stores the translation in item.metadata['title_zh'] and updates the DB.
        Skips if already translated or if the item is already in Chinese.
        """
        if item.language == "zh":
            return
        ...
        try:
            resp = await self.llm.chat(
                system_prompt="你是一个翻译助手。将英文新闻标题翻译为简洁准确的中文。只返回翻译结果，不要任何解释。",
                user_prompt=item.title,
                model=self.llm.model_filter,
                max_tokens=200,
                temperature=0.1,
                stage="translate",
                source_name=item.source_name,
            )
            title_zh = (resp.get("content") or "").strip()
            # Strip <think>...</think> blocks from reasoning models
            title_zh = re.sub(r"<think>.*?</think>", "", title_zh, flags=re.DOTALL).strip()
            if title_zh and len(title_zh) > 1:
                if not isinstance(item.metadata, dict):
                    item.metadata = {}
                item.metadata["title_zh"] = title_zh
                try:
                    await self.db.update_news_metadata(item.id, "title_zh", title_zh)
                except Exception as e:
                    logger.debug("[Pipeline] title_zh DB update failed: %s", e)
                logger.info("[Pipeline] Translated title: '%s' → '%s'", item.title[:50], title_zh[:50])
        except Exception as e:
            logger.warning("[Pipeline] Title translation failed: %s", e)
    ...
    async def _phase1_evaluate(
        ...
        evaluation = InitialEvaluation(
            ...
            is_neutral=bool(parsed.get("is_neutral", False)),
            is_stale=bool(parsed.get("is_stale", False)),
            estimated_publish_date=parsed.get("estimated_publish_date"),
            model_used=self.llm.model_analyzer,
        )

        logger.info(
            "[Phase1] score=%.2f affect=%s neutral=%s stocks=%s sectors=%s baidu_queries=%d google_queries=%d | %s",
            evaluation.relevance_score,
            evaluation.may_affect_market,
            evaluation.is_neutral,
    ...
    async def _phase3_assess(
        ...
        for entry in raw_per_stock:
            ...
            # Extract multi-timeframe predictions (null = model can't judge)
            has_horizons = False
            for tf in ("short_term", "medium_term", "long_term"):
                tf_data = entry.get(tf)
                if tf_data is None:
                    # Model explicitly output null — can't judge this horizon
                    parsed_entry[tf] = None
                    has_horizons = True
                elif isinstance(tf_data, dict) and tf_data.get("sentiment"):
                    sent = str(tf_data["sentiment"]).strip()
                    # Treat "neutral" sentiment as null (model should output null directly)
                    if sent == "neutral":
                        parsed_entry[tf] = None
                        has_horizons = True
                    else:
                        has_horizons = True
                        try:
                            score = max(-1.0, min(1.0, float(tf_data.get("sentiment_score", 0.0))))
                        except (ValueError, TypeError):
                            score = 0.0
                        try:
            ...
            if not has_horizons and entry.get("sentiment"):
                ...
                if fallback_sent == "neutral":
                    # All horizons null for neutral
                    for tf in ("short_term", "medium_term", "long_term"):
                        parsed_entry[tf] = None
                else:
                    # Derive numerical score from categorical label
                    score_map = {"very_bullish": 0.8, "bullish": 0.5, "bearish": -0.5, "very_bearish": -0.8}
                    fallback_score = score_map.get(fallback_sent, 0.0)
                    try:
            ...
            # Add if we have at least one horizon (including null)
    ...
    async def _build_analysis_result(
        ...
        if assessment.per_stock_sentiment:
            for pss in assessment.per_stock_sentiment:
                ...
                if matched_key:
                    # Build multi-horizon entry (null horizons are preserved)
                    entry: dict[str, Any] = {"reason": pss.get("reason", "")}
                    for tf in ("short_term", "medium_term", "long_term"):
                        if tf in pss:
                            # Store both dict (has signal) and None (can't judge)
                            entry[tf] = pss[tf]
