...
from src.utils.token_tracker import TokenTracker, estimate_messages_tokens
...
def _create_provider_client(llm_cfg: dict):
    ...
    if provider == "zhipu":
        ...
        client = AsyncOpenAI(
            ...
            http_client=httpx.AsyncClient(trust_env=False, timeout=llm_cfg.get("timeout", 60)),
        )
...
class LLMClient:
    ...
    async def _call_api(self, **kwargs) -> Any:
        """Dispatch API call to the right client."""
        if self._is_openai_compat:
            # AsyncOpenAI — already async
            return await self.client.chat.completions.create(**kwargs)
        else:
            # Zhipu — synchronous, wrap in to_thread
