...
class FeishuAlerter:
    ...
    def _build_card(
        ...
        meta = news.metadata if isinstance(news.metadata, dict) else {}
        display_title = meta.get("title_zh") or news.title
        if meta.get("title_zh") and meta["title_zh"] != news.title:
            display_title = f"{meta['title_zh']}（{news.title}）"
        title_text = f"**{display_title}**"
        if news.url:
            title_text = f"**[{display_title}]({news.url})**"
